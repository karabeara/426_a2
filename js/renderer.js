...
Renderer.onMouseUp = function( event ) {
    ...
    var mouseX = (Renderer._mouse.x + 1) / 2 * Renderer._width;
    var mouseY = (Renderer._mouse.y + 1) / 2 * Renderer._height;
    var vertDispRadius = 30;
    var vertRadius2 = (vertDispRadius/2)*(vertDispRadius/2); // HACK: hardcoded to match value in Main.createModel
    for ( var i = 0 ; i < Main.mesh.vertices.length ; i++ ) {
        var vert = Main.mesh.vertices[i];
        var raySpacePos = CopyVec(vert.position).sub(ray.origin);
        var distToRay = raySpacePos.distanceTo(CopyVec(raySpacePos).projectOnVector(ray.direction));
        var distToCamera = vert.position.distanceTo(ray.origin);
        distToCamera -= 0.05; // prioritize verts over faces

        var projector = new THREE.Projector();
        var screenPos3 = CopyVec(vert.position).project( Renderer._camera );
        var sx = (screenPos3.x + 1) / 2 * Renderer._width;
        var sy = (screenPos3.y + 1) / 2 * Renderer._height;
        var screenDist2 = (sx-mouseX)*(sx-mouseX) + (sy-mouseY)*(sy-mouseY);
    ...
    if ( nearest ) {
        var selectedFaces = Main.mesh.getSelectedFaceIds();
        var selectedVerts = Main.mesh.getSelectedVertexIds();
        var listToModify = nearestIsFace ? selectedFaces : selectedVerts;
